...
void read_ssf_from_file (circle *tree, char filename[]);
/*reads the from file into circle tree. */
...
void print_tree_to_file(circle *tree, char file[]);
/*Prints the circle "tree" in SSF format with index starting from 1 to the file*/

void print_tree(node *tree);
/*Prints the node "tree" in SSF format with index starting from 1*/

void print_tree_to_fp(node * tree,FILE *f);
/*prints the datastructre into the filepointer */

int  traverse_tree(circle *start,node ** temp,int count);
/* applies the function on all the sentences in the file*/

int sentences(circle * start);
/* returns the number of sentences in the file*/

int nodes_tb(circle *start,node ** temp,int count,int tb_start,int tb_end);
/* returns the nodes of the particular tb values */

node * particular_node(circle * start,int docid,int bodyid,int tbid,int sentenceid);
/* returns the node with a particular docid,bodyid,tbid,sentenceid)*/
