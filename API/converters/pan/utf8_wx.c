...
/*
		...
		for(k=0;k<=15;k++){if(vow_hnd[k] == ch_next){vow_ind=k;flag_vow=1;}}
		for(k=0;k<=15;k++){if(vow_hnd[k] == ch_next_next){next_vow_ind=k;next_flag_vow=1;}}
		if (flag_first_vow) {
			printf ("%c", vow_eng[first_vow_ind]);
		}
