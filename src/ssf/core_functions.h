			...
			case ' ':
				...
				if ( str1[0]!='\0' && str2[0]!='\0')
				{
					...
					else
					{
						...
						g_hash_table_insert( tmp->fs[tmp->fs_count-1]->hash, g_strdup("cm"), g_strdup(attr[6]));	
						g_hash_table_insert( tmp->fs[tmp->fs_count-1]->hash, g_strdup("suffix"), g_strdup(attr[7]));	
					}
					str1[0]='\0';str2[0]='\0';			
			...
			case '>':
				...
				if ( str1[0]!='\0' && str2[0]!='\0')
				{
					...
					else
					{
						...
						g_hash_table_insert( fs->hash, g_strdup("cm"), g_strdup(attr[6]));	
						g_hash_table_insert( fs->hash, g_strdup("suffix"), g_strdup(attr[7]));	
					}
					str1[0]='\0';str2[0]='\0';			
	...
	if(fs!=NULL)
	{
		...
		for (i=0; i< fs->key_count; i++)
		{
			...
			else
			{
				...
				strcat (str,",");
				strcat (str, (char *)g_hash_table_lookup(fs->hash, "cm"));
				strcat (str,",");
				strcat (str, (char *)g_hash_table_lookup(fs->hash, "suffix"));
				strcat (str,"\' ");
			}
